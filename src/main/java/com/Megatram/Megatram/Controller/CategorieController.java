...
//
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "404", description = " Catégorie introuvable avec l'identifiant fourni"),
            @ApiResponse(responseCode = "500", description = " Une erreur interne est survenue")
    })
    @GetMapping("/{id}")
    public ResponseEntity<Categorie> getCategorieById(@PathVariable Long id) {
...
//
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400", description = " Données invalides. Veuillez vérifier les informations saisies"),
            @ApiResponse(responseCode = "403", description = " Accès interdit : vous n'avez pas les autorisations nécessaires"),
            @ApiResponse(responseCode = "404", description = " Catégorie introuvable avec l'identifiant fourni"),
    ...
    @Operation(summary = "Supprimer une catégorie par son identifiant")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "204", description = " Catégorie supprimée avec succès"),
            @ApiResponse(responseCode = "403", description = " Accès interdit : vous n'avez pas les autorisations nécessaires"),
            @ApiResponse(responseCode = "404", description = " Catégorie introuvable avec l'identifiant fourni"),
            @ApiResponse(responseCode = "500", description = "️ Erreur interne du serveur")
    })
    @DeleteMapping("/{id}")
    public void deleteCategorie(@PathVariable Long id) {
        categorieService.deleteCategorie(id);
